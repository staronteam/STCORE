...
bool ChatHandler::HandleFlyCommand(const char* /*args*/)
{
    ...
    chr->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
    chr->RemoveAurasByType(SPELL_AURA_MOD_SHAPESHIFT);
    chr->RemoveAurasByType(SPELL_AURA_FEIGN_DEATH);
        
        if (chr->GetTeam() == ALLIANCE ) //if alliance
       {
           chr->AddAura(32242, chr);  // ALLIANCE MOUNT
       }
       else //if horde
       {
           chr->AddAura(32246, chr);  // HORDE MOUNT
       }
