...
class boss_lady_deathwhisper : public CreatureScript
{
    public:
        ...
        {
            boss_lady_deathwhisperAI(Creature* creature) : BossAI(creature, DATA_DEATHWHISPER), summons(me)
            {
            ...
            void Reset()
            {
                ...
                uiIntroTimer = 1000;
                uiSummonWaveTimer = !bFirstSummon ? 10000 : 60000;
                uiDeathandDecayTimer = 10000;
                uiFrostBoltTimer = 10000;
                uiFrostValleyTimer = 18000;
                uiShadowBoltTimer = 6000;
                uiBerserkTimer = 600000;
                uiInsignificanceTimer = 7000;
                uiShadeTimer = 10000;
                uiDominateMindTimer = 27000;
                uiCultTimer = urand(20000, 23000);
            ...
            void JustSummoned(Creature* pSummon)
            {
                ...
                if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 120.0f, true))
            ...
            {
                if(!me->HasAura(SPELL_MANA_BARRIER) && uiPhase == 1)
                    me->CastSpell(me, SPELL_MANA_BARRIER, true);

                if(me->HasAura(SPELL_MANA_BARRIER))
                {
                    if(uiDamage > me->GetPower(POWER_MANA) && uiPhase == 1)
                    {
            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                if (uiIntroTimer <= uiDiff && bIntro && uiIntroPhase < 7)
                {
                    switch (uiIntroPhase)
                    {
                        case 1: DoScriptText(SAY_INTRO_2, me); uiIntroTimer = 10000; break;
                        case 2: DoScriptText(SAY_INTRO_3, me); uiIntroTimer = 7000;  break;
                        case 3: DoScriptText(SAY_INTRO_4, me); uiIntroTimer = 12000; break;
                        case 4: DoScriptText(SAY_INTRO_5, me); uiIntroTimer = 7000;  break;
                        case 5: DoScriptText(SAY_INTRO_6, me); uiIntroTimer = 11000; break;
                        case 6: DoScriptText(SAY_INTRO_7, me); uiIntroTimer = 20000; break;
                    }
                    ++uiIntroPhase;
                ...
                if (uiCultTimer < uiDiff)
                {
                    EmpowerCultist();
                    uiCultTimer = urand(20000, 23000);
                } else uiCultTimer -= uiDiff;

                if (uiDeathandDecayTimer < uiDiff)
                {
                    if(Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 1))
                        DoCast(target, SPELL_DEATH_AND_DECAY);
                    uiDeathandDecayTimer = 11000;
                } else uiDeathandDecayTimer -= uiDiff;

                if (uiDominateMindTimer < uiDiff)
                {
                    for (uint8 i = 1; i <= RAID_MODE(0,1,1,3); ++i)
                    {
                        if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 100.0f, true, -SPELL_DOMINATE_MIND))
                            DoCast(target, SPELL_DOMINATE_MIND);
                    }
                    DoScriptText(SAY_DOMINATE_MIND, me);
                    uiDominateMindTimer = 15000;
                } else uiDominateMindTimer -= uiDiff;
                ...
                if (uiPhase == 2)
                {
                    if (uiFrostBoltTimer < uiDiff)
                    {
                        ...
                        uiFrostBoltTimer = 10000;
                    } else uiFrostBoltTimer -= uiDiff;

                    if (uiFrostValleyTimer < uiDiff)
                    {
                        DoCast(SPELL_FROST_BOLT_VALLEY);
                        uiFrostValleyTimer = 19000;
                    } else uiFrostValleyTimer -= uiDiff;

                    if (uiInsignificanceTimer < uiDiff)
                    {
                        DoCastVictim( SPELL_INSIGNIFICANCE);
                        uiInsignificanceTimer = 8000;
                    } else uiInsignificanceTimer -= uiDiff;
                    ...
                    if (uiSummonWaveTimer < uiDiff)
                    {
                        ...
                        if (Difficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
                            RandomSpawn();
                        uiSummonWaveTimer = 60000;
                    } else uiSummonWaveTimer -= uiDiff;

                    if (uiShadeTimer < uiDiff)
                    {
                        DoCast(SPELL_SUMMON_SHADE);
                        uiShadeTimer = 15000;
                    } else uiShadeTimer -= uiDiff;
        ...
        private:
            ...
            uint8 uiIntroPhase;
            uint8 uiStage;
            uint32 uiDominateMindTimer;
            uint32 uiSummonWaveTimer;
            uint32 uiDeathandDecayTimer;
            uint32 uiFrostBoltTimer;
            uint32 uiFrostValleyTimer;
...
class npc_shade : public CreatureScript
{
    public:
        ...
        {
            npc_shadeAI(Creature* creature) : ScriptedAI(creature) { }
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new npc_shadeAI(creature);
        }
};
