...
public:
    ...
    bool OnGossipHello(Player *player, Creature *_creature)
    {
        if (player->isInCombat())
        {
            ...
            _creature->MonsterWhisper("Si v kombate. Vrat sa neskor!", player->GetGUID());
            return true;
        }

        player->SEND_GOSSIP_MENU(MSG1, _creature->GetGUID());

        player->ADD_GOSSIP_ITEM(5, "Obnovit hp", GOSSIP_SENDER_MAIN, 7001);
        player->ADD_GOSSIP_ITEM(5, "Obnovit manu", GOSSIP_SENDER_MAIN, 7002);
        player->ADD_GOSSIP_ITEM(5, "Odstranit cooldown", GOSSIP_SENDER_MAIN, 7003);
        if ( player->GetTeam() == ALLIANCE )
        {
            player->ADD_GOSSIP_ITEM(5, "Odstranit Exhaustion", GOSSIP_SENDER_MAIN, 7004);
        }
        if ( player->GetTeam() == HORDE )
        {
            player->ADD_GOSSIP_ITEM(5, "Odstranit Sated", GOSSIP_SENDER_MAIN, 7005);
        }
        player->ADD_GOSSIP_ITEM(7, "Buffy", GOSSIP_SENDER_MAIN, 7006);
    ...
    bool OnGossipSelect(Player *player, Creature *_creature, uint32 sender, uint32 action)
    {
        ...
        case 7000: //Menu - Main
                player->SEND_GOSSIP_MENU(MSG1, _creature->GetGUID());
                player->ADD_GOSSIP_ITEM(5, "Obnovit hp", GOSSIP_SENDER_MAIN, 7001);
                player->ADD_GOSSIP_ITEM(5, "Obnovit menu", GOSSIP_SENDER_MAIN, 7002);
                player->ADD_GOSSIP_ITEM(5, "Odstranit cooldown", GOSSIP_SENDER_MAIN, 7003);
                if ( player->GetTeam() == ALLIANCE )
                {
                    player->ADD_GOSSIP_ITEM(5, "Odstranit Exhaustion", GOSSIP_SENDER_MAIN, 7004);
                ...
                player->ADD_GOSSIP_ITEM(11, "Buffy", GOSSIP_SENDER_MAIN, 7006);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, _creature->GetGUID());
                break;
        ...
        case 7006:
                ...
                player->ADD_GOSSIP_ITEM(11, "<---- BACK", GOSSIP_SENDER_MAIN, 7000);
                player->ADD_GOSSIP_ITEM(11, "X    CLOSE", GOSSIP_SENDER_MAIN, 9011);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, _creature->GetGUID());
                break;
